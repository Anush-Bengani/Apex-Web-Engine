...
/* –––––––––––––––––––––––––––––
PATH RESOLUTION
Resolves relative paths for both local dev and GitHub Pages.
––––––––––––––––––––––––––––– */
...
const engineRoot = new URL(’../../’, scriptDir).href;

return (relativePath) => new URL(relativePath, engineRoot).href;
})();
...
style.textContent = `#apex-loader { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; gap: 1.5rem; background: var(--color-primary, #0A0F1E); } #apex-loader__spinner { width: 2.5rem; height: 2.5rem; border: 3px solid rgba(56, 189, 248, 0.15); border-top-color: #38BDF8; border-radius: 50%; animation: apex-spin 0.75s linear infinite; } #apex-loader__text { font-family: system-ui, sans-serif; font-size: 0.875rem; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(248, 250, 252, 0.4); } @keyframes apex-spin { to { transform: rotate(360deg); } }`;
...
el.style.cssText = `display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; padding:2rem; font-family:system-ui,sans-serif; text-align:center; background:var(--color-primary,#0A0F1E); color:rgba(248,250,252,0.6);`;
el.innerHTML = `<p style="font-size:3rem; margin-bottom:1rem;" aria-hidden="true">⚠</p> <h1 style="font-size:1.25rem; color:#F8FAFC; margin-bottom:0.75rem;">Engine Error</h1> <p style="font-size:0.875rem; max-width:40ch; line-height:1.6;">${message}</p>`;
app.appendChild(el);
};
